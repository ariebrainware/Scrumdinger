...
final class ScrumTimer: ObservableObject {
    /// A struct to keep tracking aof meeting attendees during a meeting
    struct Speaker: Identifiable {
        /// The attendee name.
        let name: String
        /// True if the attendee has completed their turn to speak.
        var isCompleted: Bool
        /// Id for Identifiable comformance.
        let id = UUID()
    }
    ...
    /// The number of seconds untill all attendees have had a turn to speak
    ...
    /// A closure that is eecuted when a new attendee begins speaking.
    ...
    /**
            Initialize a new timer. Initializing a time with not arguments creates a ScrumTimer with no attendees and zero length.
    ...
    /// Start the timer.
    func startScrum() {
        timer = Timer.scheduledTimer(withTimeInterval: frequency, repeats: true) { [weak self] timer in self?.update()
        }
        timer?.tolerance = 0.1
